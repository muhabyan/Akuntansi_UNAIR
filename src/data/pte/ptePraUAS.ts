...
const tm9: Reading = {
  ...
  "blocks": [
    ...
    },
    {
      "kind": "callout",
      "variant": "info",
      "title": "Bedah Anatomi: Kurva Permintaan Investasi",
      "text": "Jangan sekadar menghafal gambar, pahami makna di balik setiap goresan garisnya:\n\n- **Sumbu Vertikal (i %):** Merupakan Suku Bunga *(Interest Rate)*. Ini adalah 'Harga' atau 'Biaya' dari meminjam uang.\n- **Sumbu Horizontal (I):** Merupakan volume Investasi Riil (pabrik, mesin, dsb) dalam satuan mata uang.\n- **Kurva DI (Demand for Investment):** Berlereng negatif (menurun dari kiri atas ke kanan bawah). Mengapa? Karena berlakunya hukum Present Value. Makin mahal suku bunga, makin ciut nyali pengusaha untuk berutang membangun proyek.\n- **Titik 10% ke I₁:** Jika Bank Sentral menetapkan suku bunga 10%, biaya cicilan sangat mencekik. Akibatnya, pengusaha hanya berani berinvestasi di level I₁ (sangat sedikit).\n- **Titik 5% ke I₂:** Jika suku bunga didiskon menjadi 5%, kredit menjadi murah meriah! Para bos perusahaan langsung agresif memborong mesin, sehingga total investasi meroket drastis dari I₁ ke I₂.\n- **Apa yang membuat Kurva DI bergeser (Shift)?** Pergerakan di SEPANJANG garis hanya terjadi kalau Suku Bunga berubah. Tapi, jika tiba-tiba ada **Inovasi Teknologi** (contoh: AI) atau **Ekspektasi Bisnis Membaik** (pajak diturunkan, politik stabil), maka seluruh Kurva DI akan **Bergeser ke Kanan**. Artinya, *walaupun* suku bunga tetap 10%, volume investasi tiba-tiba meledak dari I₁ menjadi jauh lebih besar karena adanya optimisme raksasa."
    },
    {
      "kind": "h2",
    ...
    },
    {
      "kind": "callout",
      "variant": "info",
      "title": "Bedah Anatomi: Keynesian Cross (Model Dua Sektor)",
      "text": "Grafik ini adalah mahakarya John Maynard Keynes untuk melacak titik presisi kapan ekonomi suatu negara akan stabil. Mari bedah setiap titik krusialnya:\n\n- **Sumbu Horizontal (Y):** Merupakan Pendapatan Nasional atau Total Produksi Pabrik (GDP).\n- **Sumbu Vertikal (AE):** Merupakan *Aggregate Expenditure* atau Total Rencana Belanja (Konsumsi + Investasi).\n- **Garis 45 Derajat (Y = AE):** Ini adalah 'Garis Kesempurnaan'. Titik mana pun di sepanjang garis ini melambangkan keseimbangan absolut: **Pabrik Memproduksi (Y) sama persis dengan yang Dibeli Masyarakat (AE)**. Tidak ada sisa barang di gudang, tidak ada pembeli yang kehabisan stok.\n- **Kurva AE = C + I:** Ini adalah realitas belanja masyarakat. Titik awalnya bermula dari **(C₀ + I)** di sumbu vertikal. Mengapa tidak bermula dari 0? Karena meskipun pabrik libur total (Y=0), manusia tetap harus makan (C₀) dan pengusaha tetap mengeksekusi investasi minimum (I).\n- **Kemiringan (Slope) Kurva AE:** Kemiringannya ditentukan murni oleh angka **MPC (b)**. Karena nilai $0 < MPC < 1$, maka kurva biru AE ini akan selalu menanjak *lebih landai* dibandingkan Garis 45 derajat.\n- **Titik Silang Ekuilibrium (Y*):** Terjadi tepat saat Garis Realitas AE memotong Garis Kesempurnaan 45 derajat. \n- **Apa yang terjadi jika Meleset dari Y*?**\n  - Jika ekonomi berada di *sebelah kiri Y**: Kurva AE berada *di atas* garis 45° (Belanja > Produksi). Pabrik kelabakan karena stok digudang ludes diserbu pembeli, akibatnya pabrik akan menggenjot dan memperluas produksinya hingga kembali naik ke titik Y*.\n  - Jika ekonomi berada di *sebelah kanan Y**: Kurva AE berada *di bawah* garis 45° (Belanja < Produksi). Pabrik terpaksa merumahkan karyawan karena barang menumpuk tak laku. Produksi akan dipangkas perlahan turun menuju Y* kembali.\n- **Apa yang membuat Kurva AE Bergeser Naik?** Jika Pengusaha tiba-tiba menambah Investasi $I$, atau Konsumsi Otonom masyarakat $C₀$ meroket, maka **seluruh kurva biru AE akan bergeser paralel ke atas**. Akibatnya, titik perpotongan silang akan bergeser tajam ke kanan atas, menghasilkan Ekuilibrium $Y*$ yang jauh lebih besar (inilah perwujudan visual dari **Multiplier Effect**!)."
    },
    {
      "kind": "callout",
      "variant": "info",
