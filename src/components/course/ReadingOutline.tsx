...
function OutlineLinks({ items, activeId }: { items: ReadingOutlineItem[]; activeId?: string }) {
  return (
...
export default function ReadingOutline({ items, variant }: { items: ReadingOutlineItem[]; variant: 'mobile' | 'desktop' }) {
  const [activeId, setActiveId] = useState(items[0]?.id);

  useEffect(() => {
    setActiveId(items[0]?.id);
    if (variant !== 'desktop' || typeof IntersectionObserver === 'undefined') return;
  ...
  if (variant === 'mobile') {
    return (
      <details className="reading-outline mb-6 rounded-xl border border-gray-200 bg-white/70 dark:border-gray-700 dark:bg-gray-900/55 lg:hidden">
        ...
        <div className="border-t border-gray-200 p-2 dark:border-gray-700"><OutlineLinks items={items} /></div>
      </details>
    );
  }

  return (
    <aside className="reading-outline reading-outline--desktop sticky top-24 hidden max-h-[calc(100dvh-7rem)] self-start overflow-y-auto border-l border-gray-200 pl-4 pr-1 dark:border-gray-800 lg:block">
      <div className="mb-3 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-gray-500 dark:text-gray-400">
