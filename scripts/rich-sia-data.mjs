// scripts/rich-sia-data.mjs
// Data akademik mendalam TM 2 - TM 14 Sistem Informasi Akuntansi (Richardson 4e)

export const RICH_SIA_MODULES = [
  {
    tm: 2,
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Kategori", "Simbol", "Notasi Grafis", "Deskripsi & Implikasi Audit"],
        rows: [
          ["Start Event", "Lingkaran garis tunggal tipis", "○", "Pemicu dimulainya proses (misal: Pesanan Masuk, Faktur Diterima). Audit memeriksa cap waktu (timestamp) pemicu."],
          ["Intermediate Event", "Lingkaran garis ganda", "◎", "Kejadian di tengah proses (misal: Menunggu Pembayaran, Pengiriman Notifikasi). Menandakan jeda waktu atau syarat eksternal."],
          ["End Event", "Lingkaran garis tebal", "●", "Hasil akhir dari proses (misal: Barang Terkirim, Transaksi Dibatalkan)."],
          ["Activity / Task", "Persegi panjang sudut membulat", "[ Tugas ]", "Tindakan spesifik yang dilakukan personil atau sistem (misal: 'Verifikasi Batas Kredit', 'Rekonsiliasi Bank')."],
          ["Exclusive Gateway (XOR)", "Belah ketupat bertanda X", "◇ (X)", "Percabangan kondisi di mana HANYA SATU jalur yang dapat dipilih (misal: Kredit Disetujui ATAU Ditolak)."],
          ["Parallel Gateway (AND)", "Belah ketupat bertanda +", "◇ (+)", "Pemisahan alur menjadi beberapa jalur yang dieksekusi SECARA BERSAMAAN tanpa syarat (misal: Kirim Pesanan DAN Terbitkan Tagihan)."],
          ["Inclusive Gateway (OR)", "Belah ketupat bertanda O", "◇ (O)", "Percabangan di mana satu atau beberapa jalur dapat dipilih sekaligus berdasarkan pemenuhan syarat."],
          ["Sequence Flow", "Garis solid bertanda panah", "───►", "Menunjukkan urutan eksekusi aktivitas DALAM SATU POOL. Tidak boleh menembus batas pool."],
          ["Message Flow", "Garis putus-putus bertanda lingkaran & panah terbuka", "◌ - - - ▷", "Menunjukkan pertukaran pesan/informasi ANTARA DUA POOL TERPISAH (misal: Pelanggan ke Perusahaan)."]
        ]
      },
      {
        kind: "h2",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Komponen REA", "Definisi Konseptual", "Contoh Siklus Pendapatan", "Contoh Siklus Pengeluaran"],
        rows: [
          ["Resources (R)", "Objek bernilai ekonomi yang dimiliki atau dikendalikan entitas, yang dapat bertambah atau berkurang akibat peristiwa bisnis.", "Persediaan Barang Dagang, Kas di Bank.", "Kas di Bank, Bahan Baku, Mesin/Peralatan Pabrik."],
          ["Events (E)", "Aktivitas atau peristiwa operasional yang mengubah kuantitas atau nilai Resource ekonomi.", "Pesanan Penjualan (Commitment), Pengiriman Barang (Economic Decrement), Penerimaan Kas (Economic Increment).", "Purchase Order (Commitment), Penerimaan Barang (Economic Increment), Pengeluaran Kas (Economic Decrement)."],
          ["Agents (A)", "Individu atau organisasi yang berpartisipasi dalam perencanaan, eksekusi, atau pengawasan Event.", "Pelanggan (Eksternal), Kasir (Internal), Staf Penjualan (Internal).", "Pemasok/Vendor (Eksternal), Staf Pembelian (Internal), Petugas Gudang (Internal)."]
        ]
      },
      {
        kind: "h2",
      ...
      },
      {
        ...
        headers: ["Tipe Relasi", "Aturan Desain Database", "Contoh Kasus Akuntansi", "Implementasi Tabel"],
        rows: [
          ["1 to 1 (1:1)", "Primary Key dari salah satu entitas dijadikan Foreign Key pada entitas pasangannya.", "Pengiriman Barang ke Faktur Penjualan (bila 1 kirim = 1 faktur).", "Kolom No_Pengiriman masuk ke tabel Faktur_Penjualan."],
          ["1 to Many (1:N)", "Primary Key dari sisi '1' WAJIB menjadi Foreign Key di sisi 'Many' (N).", "Pelanggan (1) ke Pesanan Penjualan (N) — Satu pelanggan dapat melakukan banyak pesanan.", "Kolom ID_Pelanggan masuk ke tabel Pesanan_Penjualan."],
          ["Many to Many (M:N)", "Wajib membuat TABEL RELASI PERANTARA (Junction/Bridge Table) dengan Primary Key gabungan (Composite Key).", "Pesanan Penjualan (M) ke Barang Persediaan (N) — Satu pesanan bisa membeli banyak barang, satu barang bisa dipesan di banyak pesanan.", "Tabel 'Detail_Pesanan_Penjualan' dengan PK gabungan (No_Pesanan + Kode_Barang) ditambah atribut Qty dan Harga_Satuan."]
        ]
      },
      {
        kind: "callout",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Tahap", "Persyaratan Teknis", "Anomali yang Diatasi", "Tindakan Dekomposisi"],
        rows: [
          ["Unnormalized (UNF)", "Data mengandung grup berulang (repeating groups) atau multi-value attributes.", "Redundansi ekstrem, pemborosan storage.", "Satu baris faktur memuat beberapa baris barang."],
          ["1st Normal Form (1NF)", "Setiap sel tabel hanya memuat satu nilai atomik (atomic value), tidak ada repeating groups, telah ditentukan Primary Key.", "Mencegah kesalahan indeksasi baris.", "Pisahkan baris barang sehingga setiap baris hanya memuat satu kombinasi transaksi dan produk."],
          ["2nd Normal Form (2NF)", "Telah 1NF DAN setiap atribut non-kunci bergantung PENUH pada SELURUH Primary Key (bebas Partial Dependency).", "Update Anomaly (mengubah nama produk harus mengedit ribuan baris faktur lama).", "Pisahkan atribut produk (Nama_Barang, Harga_Standar) ke tabel Master_Produk tersendiri."],
          ["3rd Normal Form (3NF)", "Telah 2NF DAN tidak ada ketergantungan transitif (bebas Transitive Dependency; X -> Y -> Z).", "Deletion Anomaly (menghapus faktur terakhir seorang pelanggan menyebabkan seluruh profil pelanggan terhapus).", "Pisahkan profil pelanggan (Nama, Alamat, Syarat Kredit) ke tabel Master_Pelanggan tersendiri."]
        ]
      },
      {
        kind: "h2",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Tahap Proses", "Risiko Utama", "Pengendalian Kunci (COSO)", "Dokumen Terkait"],
        rows: [
          ["Pesanan Penjualan", "Menerima pesanan dari pihak fiktif atau pesanan salah ketik.", "Verifikasi identitas pelanggan, pesanan bernomor urut pracetak (prenumbered).", "Customer PO, Sales Order."],
          ["Persetujuan Kredit", "Penjualan kepada pelanggan dengan risiko kredit macet tinggi.", "Pemisahan Bagian Kredit dari Penjualan; batas kredit diotorisasi manajemen.", "Credit Approval Log."],
          ["Pengiriman Barang", "Pencurian barang gudang atau salah kirim kuantitas/spesifikasi.", "Pemeriksaan independen di dermaga pengiriman; rekonsiliasi Picking List vs Packing Slip.", "Picking Ticket, Packing Slip, Bill of Lading."],
          ["Penagihan Faktur", "Kegagalan menagih (lupa buat faktur) atau salah harga.", "Sistem otomatis menerbitkan faktur saat Bill of Lading berstatus terkirim; daftar harga master resmi terkunci.", "Sales Invoice, Sales Journal."],
          ["Penerimaan Kas", "Penggelapan uang tunai oleh staf (Theft of Cash, Lapping).", "Gunakan sistem Bank Lockbox; pemisahan fungsi penerima kas dari pencatat buku pembantu piutang.", "Remittance Advice, Deposit Slip, Bank Statement."]
        ]
      },
      {
        kind: "h2",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Nama Dokumen", "Pihak Penerbit", "Informasi Kritis yang Diverifikasi"],
        rows: [
          ["Purchase Order (PO)", "Departemen Pembelian", "Kuantitas resmi yang disetujui, harga satuan kontrak, termin pembayaran, spesifikasi barang."],
          ["Receiving Report", "Departemen Penerimaan", "Kuantitas fisik aktual yang diterima di dermaga, tanggal penerimaan, kondisi barang (rusak/baik)."],
          ["Vendor Invoice", "Pemasok / Vendor Luar", "Jumlah tagihan kotor, diskon tunai, ongkos kirim, dan nomor rekening tujuan."]
        ]
      },
      {
        kind: "p",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Siklus Transaksi", "Peristiwa Kunci (Event)", "Titik Otorisasi Kritis", "Pemisahan Tugas Wajib (SoD)"],
        rows: [
          ["Siklus Pendapatan (O2C)", "Pengiriman Barang & Penagihan Faktur", "Otorisasi Kredit oleh Bagian Kredit sebelum barang dikeluarkan.", "Penjualan vs Kredit vs Kasir vs Pembukuan Piutang."],
          ["Siklus Pengeluaran (P2P)", "Penerimaan Barang & Pengeluaran Kas", "Persetujuan PO oleh Purchasing; Otorisasi Voucher oleh AP.", "Purchasing vs Receiving vs AP vs Treasury (Kasir)."],
          ["Siklus Konversi", "Pengeluaran Bahan Baku & Penyelesaian Unit", "Penerbitan Production Order oleh Bagian Perencanaan Produksi (PPIC).", "Gudang Bahan Baku vs Operator Produksi vs Akuntansi Biaya."]
        ]
      },
      {
        kind: "h2",
      ...
      },
      {
        ...
        headers: ["Kelemahan Pengendalian (Finding)", "Risiko / Potensi Kerugian (Risk)", "Rekomendasi Perbaikan (Remedy)"],
        rows: [
          ["Staf penjualan berwenang menyetujui kredit dan menghapus piutang.", "Piutang macet melonjak dan potensi penggelapan kas dengan menghapus piutang sepihak.", "Bentuk Departemen Kredit terpisah; penghapusan piutang wajib otorisasi Dewan Direksi."],
          ["Staf penerimaan menerima salinan PO lengkap dengan kuantitas barang.", "Staf tidak menghitung fisik barang dan berpotensi menerima barang kurang dari vendor.", "Terapkan Blind Receiving Report (kolom kuantitas dikosongkan)."],
          ["Kasir merangkap pemegang buku pembantu piutang dan rekonsiliasi bank.", "Risiko kecurangan Lapping dan pencurian uang kas tanpa terdeteksi.", "Pisahkan kasir (penerima fisik) dari pencatat piutang dan tugaskan staf independen untuk rekonsiliasi bank bulanan."]
        ]
      },
      {
        kind: "callout",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Komponen COSO", "Fokus Utama", "Contoh Implementasi Praktis"],
        rows: [
          ["1. Control Environment (Lingkungan Pengendalian)", "Fondasi organisasi, integritas, nilai etika, struktur pelaporan, dan komitmen terhadap kompetensi ('Tone at the Top').", "Penerbitan Kode Etik Karyawan, Whistleblowing System independen, Komite Audit yang aktif."],
          ["2. Risk Assessment (Penilaian Risiko)", "Proses dinamis untuk mengidentifikasi dan menilai risiko ketercapaian tujuan, termasuk risiko fraud dan perubahan teknologi.", "Penyusunan Risk Register tahunan, analisis kerentanan sistem siber, evaluasi risiko kecurangan pelaporan keuangan."],
          ["3. Control Activities (Aktivitas Pengendalian)", "Tindakan yang ditetapkan melalui kebijakan dan prosedur untuk memitigasi risiko hingga tingkat yang dapat diterima.", "Pemisahan tugas (SoD), otorisasi ganda transaksi di atas Rp 100 juta, pembatasan akses fisik ruang server."],
          ["4. Information & Communication", "Penyediaan informasi berkualitas yang relevan dan tepat waktu, serta saluran komunikasi dua arah (internal dan eksternal).", "Sistem pelaporan manajerial real-time, manual kebijakan akuntansi yang terdistribusi, pengaduan konsumen."],
          ["5. Monitoring Activities (Pemantauan)", "Evaluasi berkala dan berkelanjutan untuk memastikan kelima komponen pengendalian berfungsi efektif.", "Audit internal rutin, audit penetration testing berkala, tindak lanjut atas temuan auditor eksternal."]
        ]
      },
      {
        kind: "h2",
      ...
      },
      {
        ...
        headers: ["Klasifikasi", "Ambang Batas Keparahan", "Kewajiban Pelaporan"],
        rows: [
          ["Control Deficiency (Defisiensi Pengendalian)", "Rancangan atau operasi pengendalian tidak memungkinkan personel mencegah atau mendeteksi salah saji tepat waktu dalam kegiatan normal.", "Dilaporkan ke manajemen operasional dalam Management Letter."],
          ["Significant Deficiency (Defisiensi Signifikan)", "Defisiensi yang cukup penting untuk mendapatkan perhatian dari pihak yang bertanggung jawab atas tata kelola (Komite Audit/Dewan Komisaris), namun tidak berpotensi salah saji material.", "Wajib dilaporkan secara tertulis kepada Komite Audit dan Dewan Direksi."],
          ["Material Weakness (Kelemahan Material)", "Tingkat defisiensi paling parah di mana terdapat kemungkinan masuk akal (reasonable possibility) bahwa salah saji material pada laporan keuangan tidak dapat dicegah atau dideteksi tepat waktu.", "Wajib dipublikasikan dalam opini audit publik SOX 404 dan menyebabkan Opini Tidak Wajar (Adverse Opinion) atas ICFR."]
        ]
      },
      {
        kind: "callout",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Karakteristik", "Kriptografi Simetris (Secret Key)", "Kriptografi Asimetris (Public-Private Key)"],
        rows: [
          ["Kunci Enkripsi & Dekripsi", "Menggunakan SATU kunci rahasia yang sama untuk enkripsi dan dekripsi.", "Menggunakan SEPASANG kunci: Public Key (dibagikan bebas) dan Private Key (disimpan rahasia)."],
          ["Kecepatan Komputasi", "Sangat cepat, cocok untuk mengenkripsi database bervolume gigabyte/terabyte.", "Lebih lambat (sekitar 1.000x lebih lambat dari simetris), hanya efisien untuk data kecil."],
          ["Masalah Utama", "Distribusi kunci: bagaimana membagikan kunci rahasia secara aman kepada pihak kedua tanpa disadap?", "Distribusi kunci mudah melalui Public Key, namun membutuhkan infrastruktur sertifikat digital (PKI)."],
          ["Penerapan Akuntansi", "Enkripsi seluruh database ERP yang sedang tersimpan (Data at Rest) menggunakan standar AES-256.", "Tanda tangan digital (Digital Signature) untuk otorisasi transfer kawat dan validasi faktur elektronik pajak."]
        ]
      },
      {
        kind: "p",
      ...
      },
      {
        ...
        headers: ["Parameter", "Hot Site", "Warm Site", "Cold Site"],
        rows: [
          ["Biaya Investasi", "Sangat Mahal (operasi 24/7 paralel).", "Moderat (perangkat keras siap, data dipulihkan terjadwal).", "Paling Ekonomis (hanya sewa gedung kosong)."],
          ["Recovery Time Objective (RTO) — Berapa lama sistem boleh mati?", "Hitungan menit hingga detik (hampir instan).", "Hitungan beberapa jam hingga 1 hari.", "Hitungan hari hingga minggu."],
          ["Recovery Point Objective (RPO) — Berapa jam transaksi yang boleh hilang?", "Mendekati 0 detik (data direplikasi secara sinkron real-time).", "Maksimal data backup hari sebelumnya (misal 24 jam).", "Seluruh data sejak backup tape fisik terakhir."],
          ["Kelayakan Penggunaan", "Bank, bursa efek, sistem pembayaran nasional.", "Perusahaan manufaktur dan distribusi skala menengah.", "Organisasi dengan operasional non-kritis."]
        ]
      },
      {
        kind: "callout",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Nama Pengendalian", "Aturan Logika", "Contoh Kasus Kesalahan yang Dicegah"],
        rows: [
          ["Field Check / Type Check", "Memastikan karakter yang dimasukkan memiliki tipe data yang benar.", "Mencegah entri huruf pada kolom 'Jumlah Pembayaran'."],
          ["Sign Check", "Memeriksa tanda aritmatika (positif/negatif) apakah logis.", "Mencegah entri kuantitas jam kerja bernilai negatif."],
          ["Limit Check & Range Check", "Memastikan nilai berada di bawah batas tertentu atau dalam rentang yang wajar.", "Menolak entri jam lembur > 20 jam per minggu atau nilai gaji < UMR."],
          ["Completeness Check", "Memastikan tidak ada field wajib yang dibiarkan kosong.", "Menolak menyimpan faktur jika nomor NPWP pelanggan belum diisi."],
          ["Validity Check", "Mencocokkan nilai input dengan data master yang valid.", "Menolak entri transaksi untuk Kode Akun yang tidak terdaftar di Bagan Akun (COA)."],
          ["Check Digit Verification", "Menghitung angka penguji matematis pada digit terakhir nomor identifikasi.", "Mencegah salah ketik nomor rekening bank atau nomor kartu kredit."]
        ]
      },
      {
        kind: "h2",
      ...
      },
      {
        ...
        headers: ["Dimensi", "Audit Around the Computer (Black Box)", "Audit Through the Computer (White Box)"],
        rows: [
          ["Fokus Audit", "Mencocokkan input fisik (dokumen sumber) langsung dengan output laporan tanpa menguji logika program.", "Menguji integritas logika program, algoritma penghitungan, dan pengendalian otomatis di dalam software."],
          ["Kapan Digunakan?", "Sistem sederhana, volume transaksi rendah, jejak audit (audit trail) fisik tersedia lengkap.", "Sistem kompleks, transaksi real-time tanpa kertas (paperless), pemrosesan cloud ERP berskala besar."],
          ["Risiko Utama", "Tidak mendeteksi kecurangan logika program yang disisipkan programmer (misal: Salami Fraud yang memotong sisa desimal sen).", "Membutuhkan keahlian teknis auditor yang tinggi dan perangkat lunak audit khusus (CAATs)."]
        ]
      },
      {
        kind: "h2",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Tingkatan Analitika", "Pertanyaan Inti", "Contoh Kasus Akuntansi Manajemen", "Contoh Kasus Audit"],
        rows: [
          ["Descriptive Analytics", "Apa yang telah terjadi? (What happened?)", "Laporan ringkasan penjualan per wilayah, analisis rasio likuiditas historis.", "Tabel profil umur piutang (Aging Schedule), total pengeluaran kas tahun berjalan."],
          ["Diagnostic Analytics", "Mengapa hal itu terjadi? (Why did it happen?)", "Analisis varians biaya bahan baku aktual vs standar, drill-down penyebab lonjakan retur.", "Deteksi transaksi akhir pekan/hari libur, pemindaian nomor cek yang melompat (gap detection)."],
          ["Predictive Analytics", "Apa yang kemungkinan akan terjadi? (What is likely to happen?)", "Peramalan arus kas 6 bulan ke depan menggunakan time-series, estimasi probabilitas kebangkrutan (Altman Z-Score).", "Model klasifikasi machine learning untuk memprediksi risiko kegagalan bayar debitur."],
          ["Prescriptive Analytics", "Tindakan apa yang sebaiknya diambil? (What should we do?)", "Optimasi rute distribusi logistik untuk meminimalkan biaya bahan bakar, penetapan harga dinamis (dynamic pricing).", "Sistem rekomendasi otomatis untuk membekukan akun vendor yang memiliki anomali skor risiko tinggi."]
        ]
      },
      {
        kind: "h2",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Aktivitas Akuntansi", "Metode Konvensional", "Era AI & Otomasi"],
        rows: [
          ["Input Faktur Vendor", "Mengetik manual nomor faktur, tanggal, dan nominal item per item.", "Intelligent Document Processing (IDP/OCR) memindai PDF faktur dan mengisi field ERP otomatis."],
          ["Rekonsiliasi Bank", "Pencocokan manual manual baris demi baris menggunakan pulpen stabilo.", "Bot RPA mencocokkan 95% transaksi otomatis; manusia hanya menangani 5% sisa pengecualian."],
          ["Pendeteksian Fraud", "Pemeriksaan acak berdasarkan sampel audit tahunan.", "Machine learning memantau 100% aliran transaksi real-time dan memberikan peringatan fraud seketika."],
          ["Fokus Nilai Tambah", "Pencatatan data klerikal (Data Processing).", "Interpretasi wawasan strategis, penilaian risiko, dan pengambilan keputusan (Decision Support)."]
        ]
      },
      {
        kind: "callout",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Era Akuntansi", "Mekanisme Pembukuan", "Kelemahan & Kebutuhan Audit", "Pihak yang Memverifikasi"],
        rows: [
          ["Single-Entry (Pra-1494)", "Hanya mencatat penambahan atau pengurangan kas dalam satu daftar.", "Tidak ada mekanisme kontrol internal keseimbangan; mudah terjadi kehilangan catatan.", "Pencatat itu sendiri (sangat subjektif)."],
          ["Double-Entry (Luca Pacioli 1494)", "Setiap transaksi dicatat sebagai Debit dan Kredit pada dua buku terpisah milik masing-masing entitas.", "Masing-masing pihak (penjual dan pembeli) memegang buku sendiri; rawan perbedaan saldo dan memerlukan konfirmasi audit eksternal.", "Auditor eksternal independen melakukan audit sampling atas dokumen kertas."],
          ["Triple-Entry (Era Blockchain)", "Selain debit pada pembeli dan kredit pada penjual, entri KETIGA dicatat secara kriptografis pada Shared Distributed Ledger publik.", "Data diverifikasi otomatis oleh jaringan komputer; tidak ada perselisihan saldo antar pihak; biaya audit rekonsiliasi terpangkas drastis.", "Konsensus kriptografi matematis jaringan (Trustless Network)."]
        ]
      },
      {
        kind: "h2",
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ["Siklus Asal", "Pemicu Transaksi (Trigger)", "Jurnal Otomatis yang Terbentuk", "Modul ERP Terkait"],
        rows: [
          ["Siklus Pendapatan (O2C)", "Konfirmasi Bill of Lading pengiriman barang dagang ke pelanggan.", "Debit: Piutang Usaha / Kas; Kredit: Pendapatan Penjualan. Debit: Beban Pokok Penjualan; Kredit: Persediaan Barang Jadi.", "SD (Sales & Distribution) -> FI (Financial Accounting) & MM (Materials Management)."],
          ["Siklus Pengeluaran (P2P)", "Three-Way Match disetujui (PO + RR + Vendor Invoice).", "Debit: Persediaan Bahan Baku / Beban Operasional; Kredit: Utang Usaha.", "MM (Materials Management) -> FI (Accounts Payable)."],
          ["Siklus Konversi", "Laporan penyelesaian batch produksi dari stasiun perakitan.", "Debit: Persediaan Barang Jadi; Kredit: Barang Dalam Proses (WIP).", "PP (Production Planning) -> CO (Controlling) & FI."],
          ["Siklus Penggajian (Payroll)", "Otorisasi rekapitulasi jam kerja biometrik bulanan.", "Debit: Beban Gaji & Upah; Kredit: Utang PPh 21, Utang BPJS, dan Kas/Utang Gaji.", "HR/HCM (Human Capital) -> FI."]
        ]
      },
      {
        kind: "h2",
      ...
      },
      {
        ...
        headers: ["Kasus", "Kelemahan Pengendalian", "Kategori COSO / GITC", "Solusi & Pengendalian Perbaikan"],
        rows: [
          ["Temuan 1", "Ketiadaan perencanaan kapasitas server dan tidak adanya High Availability / Hot Site.", "GITC (Computer Operations & Availability)", "Migrasikan database ke arsitektur Cloud ERP yang memiliki auto-scaling; siapkan server replikasi aktif."],
          ["Temuan 2", "Pelanggaran serius pemisahan tugas (SoD) antara pendaftaran vendor dan pemesanan.", "Control Activities (Segregation of Duties)", "Cabut hak akses otorisasi PO dari staf pendaftaran vendor; terapkan matriks SoD di konfigurasi modul ERP."],
          ["Temuan 3", "Fraud Vendor Fiktif (Shell Company) dan transaksi dengan pihak berelasi tanpa deklarasi.", "Risk Assessment & Control Environment", "Wajibkan verifikasi fisik dan legalitas NPWP/NIB vendor oleh Bagian Legal; terapkan audit data matching alamat vendor vs karyawan."],
          ["Temuan 4", "Ketiadaan Data Loss Prevention (DLP) dan akses fisik port USB yang terbuka bebas.", "Information Security (GITC - Logical Access)", "Nonaktifkan seluruh port USB di komputer kantor melalui Group Policy Windows; enkripsi harddisk laptop dengan BitLocker; terapkan perjanjian kerahasiaan (NDA)."]
        ]
      },
      {
        kind: "callout",
