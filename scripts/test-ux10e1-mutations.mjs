...
const testScript = 'scripts/test-ux10e1-findings.mjs';
const mutations = [
  {
    id: 'current_card_reintroduced_as_fallback',
    mode: 'dynamic',
    file: 'src/components/FlashcardDeck.tsx',
    from: '{ length: totalCards - 1 }',
    to: '{ length: totalCards }',
  },
  {
    id: 'reduced_motion_logic_branch_removed',
    mode: 'dynamic',
    file: 'src/components/FlashcardDeck.tsx',
    from: 'if (prefersReducedMotion) {',
    to: 'if (false) {',
  },
];
...
function executeMutation(tempRoot) {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [path.join(tempRoot, testScript)], {
      cwd: tempRoot,
      env: { ...process.env, UX10E1_ROOT: tempRoot, UX10E1_DYNAMIC_ONLY: '1' },
...
async function runMutation(mutation) {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), `ux10e1-${mutation.id}-`));
  try {
