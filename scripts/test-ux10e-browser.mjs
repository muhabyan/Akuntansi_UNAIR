...
const evidenceDir = path.join(root, 'UX1_0E_EVIDENCE', 'browser');
...
const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'ux10e-chromium-'));
const results = [];
try {
  ...
  fs.writeFileSync(path.join(evidenceDir,'UX1_0E_BROWSER.json'), JSON.stringify(report,null,2)); console.log(JSON.stringify(report,null,2));
} catch (error) {
  ...
  fs.writeFileSync(path.join(evidenceDir,'UX1_0E_BROWSER.json'), JSON.stringify(report,null,2)); console.error(JSON.stringify(report,null,2)); process.exitCode = blocked ? 2 : 1;
} finally {
